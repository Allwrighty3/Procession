defmodule Procession.Dialogue.Context do
  ...
  def from_session_state(state, target_id, message, opts \\ [])

  def from_session_state(%GameSession{} = state, target_id, message, opts)
      when is_binary(target_id) and is_binary(message) and is_list(opts) do
    with true <- target_id in state.active_entities,
         true <- EntitySupervisor.exists?(target_id),
         {:ok, player_id} <- session_state_player(state),
         true <- player_id in state.active_entities,
         true <- EntitySupervisor.exists?(player_id),
         {:ok, target_state} <- get_entity_state(target_id),
         {:ok, speaker_state} <- get_entity_state(player_id),
         {:ok, location_context} <- current_location_context_from_state(state),
         {:ok, active_entity_context} <- active_entities_context_from_ids(state.active_entities),
         {:ok, target_memories} <- target_memories(target_id, opts) do
      {:ok,
       %{
         target: entity_facts(target_state),
  ...
  def from_session_state(_state, _target_id, _message, _opts) do
    {:error, :invalid_dialogue_context}
  end
  ...
  defp active_entities_context(session) do
    ...
    |> active_entities_context_from_ids()
  end
  ...
  defp session_state_player(%GameSession{player_id: nil}), do: {:error, :player_not_found}

  defp session_state_player(%GameSession{player_id: player_id}) when is_binary(player_id) do
    {:ok, player_id}
  end

  defp current_location_context_from_state(state) do
    with {:ok, player_id} <- session_state_player(state),
         true <- EntitySupervisor.exists?(player_id),
         {:ok, player_state} <- get_entity_state(player_id),
         location_id when is_binary(location_id) <- player_state.location,
  ...
  defp active_entities_context_from_ids(entity_ids) do
    entity_ids
    |> Enum.reduce({:ok, []}, fn
