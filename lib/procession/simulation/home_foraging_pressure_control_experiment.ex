defmodule Procession.Simulation.HomeForagingPressureControlExperiment do
  @moduledoc "Ultra-slow home-foraging under restored physical pressure and no-teacher control."
  ...
  def run(opts \\ []) do
    ...
    stage_ticks = Keyword.get(opts, :stage_ticks, @stage_ticks)
    withdrawal_ticks = Keyword.get(opts, :withdrawal_ticks, @withdrawal_ticks)

    taught =
      for profile <- Map.keys(@profiles), condition <- [:abrupt_assistance, :staged_assistance],
  ...
  def report(result) do
    taught =
      for profile <- Map.keys(@profiles), condition <- [:abrupt_assistance, :staged_assistance] do
        line(profile, condition, result.summary[{profile, condition}], result.population)
      end

    control = line(:ultra_forgiving, :no_teacher,
      result.summary[{:ultra_forgiving, :no_teacher}], result.population)

    Enum.join([
      "Ultra-slow home-foraging pressure ladder",
      "all cohorts: 0.01x motor learning, 5x1000 teaching ticks, 3000 withdrawal ticks",
      "teacher control differs only by assistance; physics and seeds match ultra_forgiving"
      | taught ++ [control]
    ], "\n")
  end

  defp line(profile, condition, s, population) do
  ...
  defp run_one(profile, condition, entity, seed, stage_ticks, withdrawal_ticks) do
    ...
    final = Enum.reduce_while(1..total, initial, fn tick, prior ->
      ...
      state = if stage == :withdrawal and prior.tick == stage_ticks * 5,
      ...
      coherence = coherence(action, event, movement, before_distance, after_distance,
      ...
      record = %{tick: tick, stage: stage, action: action, position: position,
        food: food, carrying: carrying, event: event, movement: movement}
    ...
    %{profile: profile, condition: condition, entity: entity,
      ...
      consumed: cycles != [], repeated: length(cycles) >= 2,
      cycle_count: length(cycles), first_cycle: first_or_zero(ticks),
      gap: median_gaps(ticks), collections: collections,
      unclosed: max(0, collections - length(cycles)),
  ...
  defp choose(state, hunger, cold, tick, seed, opts) do
    ...
    |> Enum.map(fn action ->
      ...
      {action, noise + max(hunger, cold) * 0.22 + Field.output_score(state.field, action, opts) * 0.65}
    end)
  ...
  defp assist(:staged_assistance, stage, intended, state, food, hunger, cold)
    ...
    level = case stage do
      :full_guidance -> 1.0
      :co_produced -> if intended == target, do: 0.55, else: 0.80
      :local_independent -> if intended == target, do: 0.30, else: 0.60
      :guided_approach -> if intended == target, do: 0.20, else: 0.45
      :near_independent -> if intended == target, do: 0.10, else: 0.30
    end
    %{action: target, level: level}
  end
  ...
  defp coherence(a, _, :moved, b, c, _, _) when a in [:north, :south, :east, :west] and c < b, do: 0.8
  defp coherence(a, _, :moved, b, c, _, _) when a in [:north, :south, :east, :west] and c > b, do: -0.6
  ...
  defp food_cell(s) when s in [:full_guidance, :co_produced], do: {3, 0}
  defp food_cell(s) when s in [:local_independent, :guided_approach], do: {3, 2}
  ...
  defp repeat_fraction(records, predicate) do
    ...
    count = Enum.count(pairs, fn [a, b] -> predicate.(a) and a.action == b.action end)
    if pairs == [], do: 0.0, else: count / length(pairs)
  end
  defp entropy([]), do: 0.0
  defp entropy(records) do
    total = length(records) * 1.0
    records |> Enum.frequencies_by(& &1.action)
  ...
  defp median(values) do
    ...
    m = div(length(sorted), 2)
    if rem(length(sorted), 2) == 1, do: Enum.at(sorted, m),
