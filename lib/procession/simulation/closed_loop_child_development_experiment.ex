defmodule Procession.Simulation.ClosedLoopChildDevelopmentExperiment do
  ...
  def report(result) do
    ...
    Enum.join(lines ++ Enum.map(@policies, &condition_line(&1, Map.fetch!(result.conditions, &1))), "\n")
  end
  ...
  defp run_entity(initial_policy, later_policy, phase_ticks, seed, entity, opts) do
    ...
    initial = %{field: DevelopmentalField.new(field_opts), capacity: 0.72, arousal: 0.28,
      proximity: 0.25, caregiver_present?: false, previous_action: :still, actions: [],
      caregiver_responses: 0, phase_one_end: nil}

    phase_one = Enum.reduce(1..phase_ticks, initial, fn tick, state ->
    ...
    final = Enum.reduce((phase_ticks + 1)..(phase_ticks * 2), phase_one, fn tick, state ->
      advance(state, tick, later_policy, rng_seed, field_opts)
    end)

    %{initial_policy: initial_policy, later_policy: later_policy,
      phase_one: phase_one_snapshot, phase_two: observe_phase(final),
      support_fingerprint: support_fingerprint(final.field),
      edge_fingerprint: edge_fingerprint(final.field), profile: structural_profile(final.field)}
  end
  ...
  defp advance(state, tick, policy, seed, field_opts) do
    ...
    %{state | field: field, capacity: capacity, arousal: arousal, proximity: proximity,
      caregiver_present?: caregiver_present?, previous_action: action,
      actions: [action | state.actions],
      caregiver_responses: state.caregiver_responses + if(response?, do: 1, else: 0)}
  end

  defp choose_action(state, tick, seed) do
    generated_bias = state.field.activity
  ...
  defp caregiver_response?(:responsive, action, state, _tick, _seed),
    do: action in [:signal, :approach] and (state.arousal >= 0.42 or state.capacity <= 0.58)
  ...
  defp caregiver_response?(:aversive, action, state, _tick, _seed),
    do: action in [:signal, :approach] and (state.arousal >= 0.42 or state.capacity <= 0.58)
  ...
  defp environmental_effect(_policy, :withdraw, false, present, state),
    do: {0.01, -0.03, if(present, do: max(state.proximity - 0.20, 0.0), else: 0.0)}
  defp environmental_effect(_policy, _action, false, true, _state), do: {0.03, -0.02, 0.75}
  defp environmental_effect(_policy, _action, false, false, state),
    do: {0.0, 0.0, max(state.proximity - 0.08, 0.0)}

  defp observe_phase(state) do
    action_counts = Enum.frequencies(state.actions)
    total_actions = max(length(state.actions), 1)
    %{generated: MapSet.size(state.field.generated), edges: map_size(state.field.edges),
      capacity: state.capacity, arousal: state.arousal,
  ...
  defp profile_similarity(left, right) do
    [sequence_similarity(left.profile.support_sizes, right.profile.support_sizes),
      ...
      sequence_similarity(left.profile.formed, right.profile.formed)] |> mean()
  end
  ...
  defp pair_mean(runs, comparer), do: (for {left, index} <- Enum.with_index(runs), right <- Enum.drop(runs, index + 1), do: comparer.(left, right)) |> mean()
