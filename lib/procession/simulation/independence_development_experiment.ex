defmodule Procession.Simulation.IndependenceDevelopmentExperiment do
  @moduledoc """
  Tests whether self-directed regulation emerges under immediate, scaffolded,
  and unavailable caregiver intervention.

  Phase one rewards :move_to_resource. Phase two invalidates that solution and
  rewards :manipulate_resource. Learned runs allow directed field activation to
  bias motor competition; matched blind runs omit that learned contribution.
  """

  alias Procession.Simulation.DevelopmentalField

  @conditions [:immediate, :scaffolded, :unavailable]
  ...
  def run(opts \\ []) do
    ...
    conditions =
      Map.new(@conditions, fn condition ->
        ...
        {condition, %{learned: summarize(learned), blind: summarize(blind), delta: delta(learned, blind)}}
      end)

    clones = Enum.map(1..2, fn _ -> run_entity(:scaffolded, phase_ticks, seed, 0, true, true) end)
  ...
  def report(result) do
    ...
    lines =
      Enum.flat_map(@conditions, fn condition ->
        result = Map.fetch!(result.conditions, condition)
        [summary_line(condition, :learned, result.learned), summary_line(condition, :blind, result.blind), delta_line(condition, result.delta)]
      end)
  ...
  defp population(condition, count, phase_ticks, seed, learned?) do
    Enum.map(1..count, fn entity -> run_entity(condition, phase_ticks, seed, entity, false, learned?) end)
  end
  ...
  defp run_entity(condition, phase_ticks, seed, entity, clone?, learned?) do
    ...
    phase_one = run_phase(initial, 1, phase_ticks, condition, :move_to_resource, rng_seed, field_opts, learned?)
    phase_two = run_phase(phase_one, phase_ticks + 1, phase_ticks, condition, :manipulate_resource, rng_seed, field_opts, learned?)
  ...
  defp run_phase(state, start_tick, phase_ticks, condition, effective_action, seed, field_opts, learned?) do
    Enum.reduce(start_tick..(start_tick + phase_ticks - 1), state, fn tick, acc ->
      advance(acc, tick, start_tick, condition, effective_action, seed, field_opts, learned?)
    end)
  end

  defp advance(state, tick, phase_start, condition, effective_action, seed, field_opts, learned?) do
    state = maybe_start_problem(state, tick, seed)
    action = choose_action(state, tick, seed, field_opts, learned?)
    self_resolved? = state.active_problem? and action == effective_action
    caregiver? = caregiver_intervenes?(condition, state, action, tick - phase_start)
    resolved? = self_resolved? or caregiver?
    ...
    features = [
      ...
      {:caregiver_intervention_channel, caregiver?},
    ...
    %{
      ...
      | field: field,
        ...
        disturbance_age: if(resolved?, do: 0, else: state.disturbance_age + bool_count(state.active_problem?)),
  ...
  defp maybe_start_problem(state, tick, seed) do
    if rem(tick + :erlang.phash2({seed, :problem_offset}, 17), 40) == 0 do
      %{state | active_problem?: true, disturbance_age: 0, attempts: 0, arousal: max(state.arousal, 0.38)}
    else
      state
    end
  end

  defp choose_action(state, tick, seed, field_opts, learned?) do
    scores =
      Map.new(@actions, fn action ->
        ...
        {action, exploration * 0.35 + urgency + learned * 1.25}
      end)

    scores |> Enum.max_by(fn {action, score} -> {score, action} end) |> elem(0)
  end

  defp urgency_score(:signal, state), do: if(state.active_problem?, do: state.arousal * 0.55, else: 0.0)
  defp urgency_score(:wait, state), do: if(state.active_problem?, do: 0.02, else: 0.35)
  defp urgency_score(_action, state), do: if(state.active_problem?, do: 0.20 + state.arousal * 0.15, else: 0.0)
  ...
  defp caregiver_intervenes?(:unavailable, _state, _action, _phase_tick), do: false
  defp caregiver_intervenes?(:immediate, state, action, _phase_tick), do: state.active_problem? and action == :signal
  defp caregiver_intervenes?(:scaffolded, state, action, phase_tick) do
    delay = cond do
      phase_tick < 250 -> 1
      phase_tick < 500 -> 2
      phase_tick < 750 -> 4
    ...
    state.active_problem? and action == :signal and state.disturbance_age >= delay
  end
  ...
  defp window_metrics(records) do
    ...
    %{
      ...
      mean_resolution_age: mean(Enum.map(Enum.filter(records, fn record -> record.resolved? end), fn record -> record.age end))
    }
  end

  defp merge_window_stats(values) do
    Map.new(Map.keys(hd(values)), fn key -> {key, stats(Enum.map(values, fn value -> Map.fetch!(value, key) end))} end)
  end
  ...
  defp delta(learned_runs, blind_runs) do
    ...
    Map.new(keys, fn key ->
      differences = Enum.map(paired, fn {learned, blind} ->
        learned_window = Map.fetch!(learned.windows, key)
        blind_window = Map.fetch!(blind.windows, key)
        Map.new(Map.keys(learned_window), fn metric -> {metric, Map.fetch!(learned_window, metric) - Map.fetch!(blind_window, metric)} end)
      end)
  ...
  defp clone_summary([left, right]) do
    %{exact: if(left.fingerprint == right.fingerprint, do: 1.0, else: 0.0), nodes: MapSet.size(left.field.generated)}
  end
  ...
  defp profile_similarity(runs) do
    ...
    if Enum.max(counts) == 0, do: 1.0, else: 1.0 - (Enum.max(counts) - Enum.min(counts)) / Enum.max(counts)
  end
  ...
  defp stats(values) do
    ...
    %{mean: avg, median: median(sorted), min: hd(sorted), max: List.last(sorted), sd: :math.sqrt(variance)}
  end

  defp median(values) do
    count = length(values)
    middle = div(count, 2)
    if rem(count, 2) == 1, do: Enum.at(values, middle), else: (Enum.at(values, middle - 1) + Enum.at(values, middle)) / 2
  end
  ...
  defp summary_line(condition, mode, summary) do
    ...
    "#{condition}_#{mode}: nodes=#{stats_text(summary.nodes)} profile=#{fmt(summary.profile_similarity)} " <>
      "final_self=#{stats_text(final.self_resolution_rate)} final_caregiver=#{stats_text(final.caregiver_rate)} " <>
      "final_signal=#{stats_text(final.signal_rate)} final_unresolved=#{stats_text(final.unresolved_rate)}"
  end

  defp delta_line(condition, windows) do
    window_text = Enum.map_join(Enum.sort(Map.keys(windows)), " ", fn key ->
      window = Map.fetch!(windows, key)
      "#{inspect(key)}:self=#{fmt(window.self_resolution_rate.mean)},care=#{fmt(window.caregiver_rate.mean)},signal=#{fmt(window.signal_rate.mean)}"
    end)
    "#{condition}_learned_minus_blind: #{window_text}"
  end

  defp stats_text(stats), do: "#{fmt(stats.mean)}[#{fmt(stats.min)}..#{fmt(stats.max)}]sd=#{fmt(stats.sd)}"
