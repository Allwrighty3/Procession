defmodule Procession.Simulation.TerrainRelaxer.Elixir do
  ...
  Each pass accumulates pairwise distance corrections, then applies them
  simultaneously. Established constraints move the neighborhood according to
  their weight while fixed regions remain anchored.
  """
  ...
  def relax(%{coordinates: coordinates, constraints: constraints} = problem, opts \\ []) do
    ...
    %{coordinates: final, residual: residual(final, constraints), iterations: iterations}
  end

  defp accumulate_corrections(coordinates, constraints, fixed, epsilon) do
    Enum.reduce(constraints, %{}, fn constraint, acc ->
      source_position = Map.fetch!(coordinates, constraint.source)
      target_position = Map.fetch!(coordinates, constraint.target)
      delta = subtract(target_position, source_position)
      actual = magnitude(delta)

      direction =
        if actual <= epsilon,
          do: fallback_direction(length(delta)),
          else: scale(delta, 1.0 / actual)

      correction = scale(direction, (actual - constraint.distance) * constraint.weight * 0.5)

      acc
      |> maybe_add(constraint.source, correction, fixed)
      |> maybe_add(constraint.target, scale(correction, -1.0), fixed)
    end)
  end

  defp maybe_add(acc, id, correction, fixed) do
    if MapSet.member?(fixed, id),
      do: acc,
      else: Map.update(acc, id, correction, &add(&1, correction))
  end
