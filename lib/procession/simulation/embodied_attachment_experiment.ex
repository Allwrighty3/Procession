defmodule Procession.Simulation.EmbodiedAttachmentExperiment do
  ...
  Caregiver contact supplies heat, provisioning, and recovery. Cue-increasing movements
  leave decaying eligibility traces that later bodily relief can reinforce.
  """
  ...
  def compare(opts \\ []) do
    ...
    %Summary{
      ...
      median_eligibility: median(Enum.map(states, &(map_size(&1.child.eligibility)))),
      median_parent_distance_before_departure:
  ...
  def report(%Summary{} = summary) do
    "samples=#{summary.samples} survived=#{summary.survived} " <>
      ...
      "resource_visits=#{fmt(summary.median_resource_visits)} " <>
      "cue_reuse=#{fmt(summary.median_cue_reuse)} memory=#{fmt(summary.median_memory)} " <>
      "eligibility=#{fmt(summary.median_eligibility)} " <>
      "pre_departure_distance=#{fmt(summary.median_parent_distance_before_departure)}"
  end
  ...
  defp advance(state, tick, opts) do
    ...
    child =
      child
      |> decay_eligibility(opts)
      |> mark_eligible(previous_cue, next_cue, action, moved?, opts)
      |> pay_movement(moved?, opts)

    baseline_unresolved = unresolved_after_regulation(child)
    ...
    delayed_relief = max(0.0, baseline_unresolved - regulated_unresolved)

    child =
      child
      |> Map.put(:unresolved, regulated_unresolved)
      |> reinforce_eligible(delayed_relief, opts)
      |> decay_memory(opts)
      |> update_independent(action, moved?, resource_id, intake, parent_present)
      |> Map.merge(%{position: next_position, motor: pressures})

    child = %{child | alive: viable?(child)}
    ...
    entry = %{
      ...
      eligibility: map_size(child.eligibility),
      memory: map_size(child.cue_memory)
    }
  ...
  defp passive_body_update(child, opts) do
    ...
    unresolved =
      child.unresolved +
        max(0.0, 0.45 - capacity) * 0.030 +
        max(0.0, 0.42 - temperature) * 0.040 +
        fatigue * 0.008 + strain * 0.010
  ...
  defp motor_pressures(state, child, cue, tick, opts) do
    ...
    effectiveness =
      child.capacity * child.temperature *
        (1.0 - child.fatigue * 0.7) *
        (1.0 - child.strain * 0.6)
  ...
  defp decay_eligibility(child, opts) do
    retention = Keyword.get(opts, :eligibility_retention, 0.88)
    ...
    %{child | eligibility: eligibility}
  end

  defp mark_eligible(child, previous_cue, next_cue, action, moved?, opts)
       when action != :rest and moved? and next_cue > previous_cue do
    key = {cue_bucket(previous_cue), action}
    cue_gain = next_cue - previous_cue
    deposit = Keyword.get(opts, :eligibility_deposit, 0.35) * cue_gain
    eligibility = Map.update(child.eligibility, key, deposit, &min(2.0, &1 + deposit))
    %{child | eligibility: eligibility}
  end

  defp mark_eligible(child, _previous_cue, _next_cue, _action, _moved?, _opts), do: child

  defp reinforce_eligible(child, relief, opts) when relief > 0.0 do
    gain = Keyword.get(opts, :cue_deposit, 1.25)

    memory =
      Enum.reduce(child.eligibility, child.cue_memory, fn {key, eligibility}, memory ->
        deposit = gain * relief * eligibility
        Map.update(memory, key, deposit, &min(2.5, &1 + deposit))
      end)

    %{child | cue_memory: memory}
  end

  defp reinforce_eligible(child, _relief, _opts), do: child

  defp decay_memory(child, opts) do
  ...
  defp pay_movement(child, true, opts) do
    ...
    %{
      child
      | capacity: clamp(child.capacity - cost),
        fatigue: clamp(child.fatigue + 0.020),
  ...
  defp update_independent(child, action, moved?, resource_id, intake, false) do
    visits =
      if is_nil(resource_id),
        do: child.independent_resource_visits,
  ...
  defp remembered?(child, action) do
    Enum.any?(child.cue_memory, fn {{_bucket, direction}, value} ->
  ...
  defp median_parent_distance(state) do
    distances =
      ...
      |> Enum.map(&manhattan(&1.child, &1.parent))
