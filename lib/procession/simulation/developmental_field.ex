defmodule Procession.Simulation.DevelopmentalField do
  ...
  defmodule Node do
    @moduledoc false
    defstruct [:id, :kind, :formed_tick, support: MapSet.new(), stability: 0.0, reuse: 0, formation_coherence: 0.0]
  end

  defmodule State do
    @moduledoc false
    defstruct tick: 0, micro_nodes: 0, next_id: 0, nodes: %{}, edges: %{}, activity: %{}, recurrence: %{}, generated: MapSet.new(), history: []
  end
  ...
  def step(%State{} = state, input, opts \\ []) do
    ...
    snapshot = %{tick: state.tick, active_micro: MapSet.size(active), generated_nodes: MapSet.size(state.generated), edge_mass: edge_mass(state.edges), active_mass: Enum.sum(Map.values(state.activity))}
    %{state | history: [snapshot | state.history]}
  end

  def run(inputs, opts \\ []), do: Enum.reduce(inputs, new(opts), fn input, state -> step(state, input, opts) end)
  ...
  defp encode({:features, features}, micro_nodes, opts) when is_list(features) do
    Enum.reduce(features, MapSet.new(), fn feature, acc -> MapSet.union(acc, encode(feature, micro_nodes, opts)) end)
  end

  defp encode(input, micro_nodes, opts) do
    width = Keyword.get(opts, :input_width, 5)
    0..(width - 1) |> Enum.map(fn offset -> :erlang.phash2({input, offset}, micro_nodes) end) |> MapSet.new()
  end

  defp update_activity(activity, active, opts) do
    retention = Keyword.get(opts, :activity_retention, 0.70)
    decayed = activity |> Enum.map(fn {id, value} -> {id, value * retention} end) |> Enum.reject(fn {_id, value} -> value < 0.001 end) |> Map.new()
  ...
  defp strengthen_edges(edges, active, opts) do
    ...
    decayed = edges |> Enum.map(fn {edge, value} -> {edge, value * retention} end) |> Enum.reject(fn {_edge, value} -> value < 0.0005 end) |> Map.new()
    active |> MapSet.to_list() |> pairs() |> Enum.reduce(decayed, fn edge, acc -> Map.update(acc, edge, gain, &min(3.0, &1 + gain)) end)
  end
  ...
  defp create_generated_node(state, active, formation_coherence) do
    id = state.next_id
    node = %Node{id: id, kind: :generated, support: active, stability: 1.0, reuse: 0, formed_tick: state.tick, formation_coherence: formation_coherence}
    edges = Enum.reduce(active, state.edges, fn member, acc -> Map.put(acc, ordered_edge(id, member), 0.25) end)
    %{state | next_id: id + 1, nodes: Map.put(state.nodes, id, node), edges: edges, generated: MapSet.put(state.generated, id)}
  end
  ...
  defp already_supported?(state, active) do
    Enum.any?(state.generated, fn id -> overlap_ratio(Map.fetch!(state.nodes, id).support, active) >= 0.90 end)
  end
  ...
  defp overlap_ratio(a, b) do
    ...
    intersection / max(MapSet.size(a), 1)
  end

  defp pairs(values), do: for({left, index} <- Enum.with_index(values), right <- Enum.drop(values, index + 1), do: ordered_edge(left, right))
