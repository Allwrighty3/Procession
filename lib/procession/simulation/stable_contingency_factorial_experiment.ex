defmodule Procession.Simulation.StableContingencyFactorialExperiment do
  @moduledoc """
  Crosses embodied and refractory motor dynamics with three field-plasticity
  profiles while the action-consequence mapping remains fixed for the whole run.

  Left output always produces a positive local consequence, right output always
  produces a negative consequence, and inactivity produces no consequence. This
  avoids spatial boundaries turning formerly useful movement into neutral output.
  """
    ...
    for motor_mode <- @motor_modes, profile <- profiles(), into: %{} do
      ...
      {key,
       %Summary{
         ...
         median_late_useful_fraction: states |> Enum.map(&late_fraction(&1, :left)) |> median(),
         median_late_harmful_fraction: states |> Enum.map(&late_fraction(&1, :right)) |> median(),
         median_late_inactive_fraction: states |> Enum.map(&late_fraction(&1, :remain)) |> median(),
         median_abandonment_events: states |> Enum.map(& &1.abandonment_events) |> median(),
         median_longest_useful_streak: states |> Enum.map(& &1.longest_useful_streak) |> median(),
         median_left_resistance:
  ...
  defp consequence(:left, opts), do: Keyword.get(opts, :useful_delta, 0.032)
  defp consequence(:right, opts), do: -Keyword.get(opts, :harmful_delta, 0.032)
  defp consequence(:remain, _opts), do: 0.0
