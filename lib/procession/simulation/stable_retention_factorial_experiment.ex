defmodule Procession.Simulation.StableRetentionFactorialExperiment do
  ...
  measures acquisition, retention, and spontaneous abandonment of useful action.
  """
  ...
  defmodule Summary do
    @moduledoc false
    defstruct [:motor_mode, :profile, :acquired, :median_acquisition_tick,
      :median_late_useful_fraction, :median_abandonment_events,
      :median_entropy, :median_left_resistance, :median_right_resistance]
  end
  ...
  def run(opts \\ []) do
    ...
    Enum.reduce(1..ticks, initial, fn tick, state ->
      advance(state, tick, ticks, Keyword.merge(profile_opts, opts))
    end)
  end
  ...
  def compare(opts \\ []) do
    ...
    for motor_mode <- @motor_modes, profile <- profiles(), into: %{} do
      ...
      {key, %Summary{
        ...
        median_late_useful_fraction: states |> Enum.map(&late_useful_fraction/1) |> median(),
        median_abandonment_events: states |> Enum.map(& &1.abandonment_events) |> median(),
        median_entropy: states |> Enum.map(&entropy/1) |> median(),
        median_left_resistance: states |> Enum.map(&CognitiveField.resistance(&1.field, :strain, :left)) |> median(),
        median_right_resistance: states |> Enum.map(&CognitiveField.resistance(&1.field, :strain, :right)) |> median()
      }}
    end
  end
    ...
    Enum.map_join(@motor_modes, "\n", fn motor_mode ->
      rows = Enum.map_join(profiles(), "\n", fn profile ->
        ...
        "  #{profile}: acquired=#{s.acquired} acquisition=#{fmt(s.median_acquisition_tick)} " <>
          "late_useful=#{fmt(s.median_late_useful_fraction)} " <>
          "abandonments=#{fmt(s.median_abandonment_events)} entropy=#{fmt(s.median_entropy)} " <>
  ...
  defp advance(state, tick, ticks, opts) do
    source = 0
    ...
    late? = tick > div(ticks, 2)
    useful_late = state.useful_late + if(late? and action == :left, do: 1, else: 0)
    harmful_late = state.harmful_late + if(late? and action == :right, do: 1, else: 0)
    ...
    %{state |
      ...
      useful_late: useful_late,
      harmful_late: harmful_late,
  ...
  defp late_useful_fraction(state) do
    total = state.useful_late + state.harmful_late
    if total == 0, do: 0.0, else: state.useful_late / total
  end
