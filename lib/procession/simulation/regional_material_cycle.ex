defmodule Procession.Simulation.RegionalMaterialCycle do
  ...
  def pressure(state) do
    ...
    energy_deficit = state.residents |> Map.values() |> Enum.map(&(1.0 - &1.energy)) |> Enum.sum() / count
  ...
  defp gather(state) do
    Enum.reduce(sorted_ids(state), {state, 0.0}, fn id, {current, total} ->
      ...
      event = {:gathered_raw, id, amount}

      {%{current | loose_raw: current.loose_raw - amount,
                    residents: Map.put(current.residents, id, updated),
                    events: [event | current.events]}, total + amount}
    end)
  end
  ...
  defp transform(state) do
    Enum.reduce(sorted_ids(state), {state, 0.0}, fn id, {current, total} ->
      ...
      event = {:transformed_material, id, amount}

      {%{current | residents: Map.put(current.residents, id, updated),
                    events: [event | current.events]}, total + amount}
    end)
  end

  defp transfer(state) do
    recipients = sorted_ids(state) |> Enum.sort_by(fn id -> {state.residents[id].energy, id} end)
    ...
    Enum.reduce(recipients, {state, 0.0}, fn recipient_id, {current, total} ->
      ...
      donor_id =
        current
        |> sorted_ids()
        |> Enum.reject(&(&1 == recipient_id))
        |> Enum.filter(&(current.residents[&1].usable > 0.08))
        |> Enum.max_by(&(current.residents[&1].usable), fn -> nil end)

      if donor_id && recipient.energy < 0.45 do
        ...
        if amount > 0.0 do
          ...
          residents = current.residents |> Map.put(donor_id, donor) |> Map.put(recipient_id, recipient)
          event = {:transferred_usable, donor_id, recipient_id, amount}
          {%{current | residents: residents, events: [event | current.events]}, total + amount}
        else
          {current, total}
        end
      else
  ...
  defp consume(state) do
    Enum.reduce(sorted_ids(state), {state, 0.0}, fn id, {current, total} ->
      ...
      event = {:consumed_usable, id, amount}
  ...
  defp held(state, key), do: state.residents |> Map.values() |> Enum.map(&Map.fetch!(&1, key)) |> Enum.sum()
  defp mean_energy(state), do: state.residents |> Map.values() |> Enum.map(& &1.energy) |> average()
  defp average([]), do: 0.0
