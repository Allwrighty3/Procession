defmodule Procession.Simulation.MultiResolutionRegion do
  ...
  def compress(%__MODULE__{resolution: resolution}, _opts)
      when resolution in [:coarse, :inert] do
    raise ArgumentError, "region is already compressed"
  end
  ...
  def make_inert(%__MODULE__{}) do
    raise ArgumentError, "only coarse regions can become inert"
  end

  def coarse_run(region, ticks, opts \\ [])

  def coarse_run(
        %__MODULE__{resolution: resolution, summary: summary} = region,
        ticks,
        opts
      )
    ...
    requested_outflow = max(0.0, Keyword.get(opts, :external_outflow, 0.0))
    satisfied_gain = max(0.0, Keyword.get(opts, :satisfied_energy_gain, 0.001))
    unmet_penalty = max(0.0, Keyword.get(opts, :unmet_energy_penalty, 0.01))
    baseline_decay = max(0.0, Keyword.get(opts, :energy_decay, 0.0015))

    updated =
      Enum.reduce(tick_steps(ticks), summary, fn _, acc ->
        requested = acc.population * demand
        stock_after_inflow = acc.available_stock + inflow
        consumed = min(stock_after_inflow, requested)
        stock_after_consumption = stock_after_inflow - consumed
        outflow = min(stock_after_consumption, requested_outflow)
        available = stock_after_consumption - outflow
  ...
  def coarse_run(%__MODULE__{}, _ticks, _opts) do
    raise ArgumentError, "only coarse or inert regions can advance coarsely"
  end

  def refine(region, seed, opts \\ [])
  ...
  def refine(
    ...
    resource_positions =
      deterministic_positions(resource_count, summary.bounds, seed + 73, summary.centroid)

    resources =
      summary.available_stock
      |> split_quantity(resource_count, seed + 61)
      |> Enum.with_index(1)
      |> Map.new(fn {quantity, index} ->
        ...
        {id,
         %{
           id: id,
           position: Enum.at(resource_positions, index - 1),
           quantity: quantity,
           signal_strength: 1.0
         }}
      end)
    ...
    %{
      ...
      | resolution: :live,
        ...
        entities: entities,
        resources: resources,
        social_relations: restore_relation_anchors(summary.relation_anchors),
        causal_flags: summary.causal_flags,
        summary: nil
    }
  end

  def refine(%__MODULE__{}, _seed, _opts) do
    raise ArgumentError, "only coarse or inert regions can be refined"
  end
  ...
  def state_cost(%__MODULE__{resolution: :live} = region) do
    map_size(region.entities) * 8 +
      map_size(region.resources) * 5 +
      map_size(region.social_relations) * 7 +
      MapSet.size(region.causal_flags)
  end
  ...
  defp normalize_entities(entities) do
    Map.new(entities, fn entity ->
      ...
      {id,
       %{
         id: id,
         position: Map.get(entity, :position, {0, 0}),
         energy: clamp(Map.get(entity, :energy, 0.75), 0.0, 1.0),
         mobility: clamp(Map.get(entity, :mobility, 1.0), 0.0, 1.0),
         inventory: max(0.0, Map.get(entity, :inventory, 0.0)),
         consumed: max(0.0, Map.get(entity, :consumed, 0.0))
       }}
    end)
  end
  ...
  defp normalize_resources(resources) do
    Map.new(resources, fn resource ->
      ...
      {id,
       %{
         id: id,
         position: Map.get(resource, :position, {0, 0}),
         quantity: max(0.0, Map.get(resource, :quantity, 0.0)),
         signal_strength: max(0.0, Map.get(resource, :signal_strength, 1.0))
       }}
    end)
  end
  ...
  defp spatial_commitment(entities) do
    ...
    spread =
      Enum.sum(
        Enum.map(entities, fn %{position: {px, py}} -> abs(px - x) + abs(py - y) end)
      ) / count
  ...
  defp split_quantity(quantity, count, seed) do
    weights =
      Enum.map(1..count, fn index ->
        1.0 + :erlang.phash2({seed, index}, 1000) / 1000
      end)
  ...
  defp distribute_mean(_target_mean, count, _seed) when count <= 0, do: []

  defp distribute_mean(target_mean, count, seed) do
    deviations =
      Enum.map(1..count, fn index ->
        (:erlang.phash2({seed, index}, 2001) - 1000) / 10_000
      end)

    centered = mean(deviations, & &1)
    values = Enum.map(deviations, &clamp(target_mean + &1 - centered, 0.0, 1.0))
    correction = target_mean - mean(values, & &1)
  ...
  defp tick_steps(0), do: []
  defp tick_steps(ticks), do: 1..ticks
