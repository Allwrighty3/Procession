defmodule Procession.AI.NPCInteraction.QE6BRelationshipExpressionSFTExporter do
  @moduledoc """
  Exports qe6b NPC interaction relationship expression SFT rows.

  qe6b trains the model to rewrite validated deterministic fallback responses
  into more character-specific dialogue using relationship profiles and relationship
  stance context.
  ...
  @default_source_paths [
    "priv/training/npc_interaction_qe6_relationship_expression_examples.jsonl",
    "priv/training/npc_interaction_qe6b_relationship_expression_patch_examples.jsonl"
  ]
  @default_output_path "priv/training/exports/npc_interaction_qe6b_relationship_expression_sft.jsonl"

  @type export_result :: {:ok, map()} | {:error, term()}

  @doc """
  Exports the default qe6 relationship expression SFT dataset.
  """
  ...
  @doc """
  Exports qe6 relationship expression SFT rows to the given output path.
  """
  @spec export(Path.t()) :: export_result()
  def export(output_path) when is_binary(output_path) do
    with {:ok, examples} <- load_examples(@default_source_paths),
  ...
  def export(_output_path), do: {:error, :invalid_qe6b_relationship_expression_sft_export_path}
  ...
  defp example_to_row(example) do
    ...
    with {:ok, pipeline_result} <- InteractionPipeline.respond(context),
      ...
      {:ok,
       %{
         "id" => "qe6b_relationship_expression_#{example["id"]}",
         "prompt" => prompt,
         "completion" => example["response"],
         "text" => prompt <> "\n" <> example["response"],
         "metadata" => %{
           "non_authoritative" => true,
           "source" => "npc_interaction_qe6b_relationship_expression_example",
           "category" => "npc_interaction_qe6b_relationship_expression",
  ...
  defp load_examples(paths) when is_list(paths) do
    paths
    |> Enum.reduce_while({:ok, []}, fn path, {:ok, examples} ->
      case VoiceExpressionExampleLoader.load(path) do
        {:ok, loaded} -> {:cont, {:ok, examples ++ loaded}}
