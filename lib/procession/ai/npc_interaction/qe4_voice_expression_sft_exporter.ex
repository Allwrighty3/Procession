defmodule Procession.AI.NPCInteraction.QE4VoiceExpressionSFTExporter do
  @moduledoc """
  Exports QE4 NPC interaction voice expression SFT rows.

  QE4 trains the model to rewrite validated deterministic fallback responses
  ...
  @default_output_path "priv/training/exports/npc_interaction_qe4_voice_expression_sft.jsonl"

  @type export_result :: {:ok, map()} | {:error, term()}

  @doc """
  Exports the default QE4 voice expression SFT dataset.
  """
  ...
  @doc """
  Exports QE4 voice expression SFT rows to the given output path.
  """
  @spec export(Path.t()) :: export_result()
  def export(output_path) when is_binary(output_path) do
    with {:ok, examples} <- VoiceExpressionExampleLoader.load_default(),
  ...
  def export(_output_path), do: {:error, :invalid_qe4_voice_expression_sft_export_path}
  ...
  defp example_to_row(example) do
    ...
    with {:ok, pipeline_result} <- InteractionPipeline.respond(context),
      ...
      {:ok,
       %{
         "id" => "qe4_voice_expression_#{example["id"]}",
         "prompt" => prompt,
         "completion" => example["response"],
         "text" => prompt <> "\n" <> example["response"],
         "metadata" => %{
           "non_authoritative" => true,
           "source" => "npc_interaction_voice_expression_example",
           "category" => "npc_interaction_voice_expression",
