defmodule Procession.AI.NPCInteraction.QE6RelationshipExpressionSFTExporter do
  @moduledoc """
  Exports qe6 NPC interaction relationship expression SFT rows.

  qe6 trains the model to rewrite validated deterministic fallback responses
  ...
  @default_source_path "priv/training/npc_interaction_qe6_relationship_expression_examples.jsonl"
  @default_output_path "priv/training/exports/npc_interaction_qe6_relationship_expression_sft.jsonl"
  ...
  def export(_output_path), do: {:error, :invalid_qe6_relationship_expression_sft_export_path}
  ...
  defp example_to_row(example) do
    ...
    prompt_opts = [
      relationship_profile: example["relationship_profile"],
    ...
    with {:ok, pipeline_result} <- InteractionPipeline.respond(context),
      ...
      {:ok,
       %{
         "id" => "qe6_relationship_expression_#{example["id"]}",
         "prompt" => prompt,
         "completion" => example["response"],
         "text" => prompt <> "\n" <> example["response"],
         "metadata" => %{
           "non_authoritative" => true,
           "source" => "npc_interaction_qe6_relationship_expression_example",
           "category" => "npc_interaction_qe6_relationship_expression",
