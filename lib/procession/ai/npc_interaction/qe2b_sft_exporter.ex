defmodule Procession.AI.NPCInteraction.QE2BSFTExporter do
  @moduledoc """
  Builds an augmented QE2b SFT dataset for NPC interaction training.
  ...
  @default_output_path "priv/training/exports/npc_interaction_qe2b_sft.jsonl"

  @type export_result :: {:ok, map()} | {:error, term()}

  @doc """
  Exports the default QE2b SFT dataset.
  """
  ...
  @doc """
  Exports the QE2b SFT dataset to the given output path.
  """
  ...
  def export(output_path) when is_binary(output_path) do
    with {:ok, base_rows} <- load_jsonl(@base_sft_path),
         ...
         rows <- build_rows(base_rows, contrastive_cases, role_boundary_examples),
  ...
  def export(_output_path), do: {:error, :invalid_qe2b_sft_export_path}

  defp build_rows(base_rows, contrastive_cases, role_boundary_examples) do
    contrastive_rows =
      contrastive_cases
    ...
    (base_rows ++ contrastive_rows ++ role_boundary_rows)
  ...
  defp contrastive_case_to_sft_row(eval_case) do
    ...
    %{
      "id" => "qe2b_contrastive_#{eval_case["id"]}",
      "text" => text,
      "metadata" => %{
  ...
  defp role_boundary_example_to_sft_row(example) do
    ...
    %{
      "id" => "qe2b_role_boundary_#{example["id"]}",
      "text" => text,
      "metadata" => %{
        "non_authoritative" => true,
