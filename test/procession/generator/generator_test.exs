defmodule Procession.GeneratorTest do
  ...
  describe "validate_blueprint/1" do
    ...
    test "generated locations include deterministic exits" do
      assert {:ok, blueprint} = Generator.generate_world("anything")

      crossroads = Enum.find(blueprint.locations, &(&1.id == "loc_crossroads"))

      assert crossroads.exits == [
               %{to: "loc_briar_village", label: "village road"},
               %{to: "loc_silent_mine", label: "mine road"}
             ]
    end

    test "validate_blueprint rejects exits to unknown locations" do
      assert {:ok, blueprint} = Generator.generate_world("anything")

      [first_location | rest_locations] = blueprint.locations

      invalid_location =
        Map.put(first_location, :exits, [
          %{to: "loc_nowhere", label: "bad road"}
        ])

      invalid_blueprint = %{blueprint | locations: [invalid_location | rest_locations]}

      assert Generator.validate_blueprint(invalid_blueprint) ==
               {:error,
                {:invalid_location_exit, invalid_location.id,
                 %{to: "loc_nowhere", label: "bad road"}, :unknown_exit_destination}}
    end

    test "spawn_world preserves location exits in metadata" do
      assert {:ok, blueprint} = Generator.generate_world("anything")

      assert {:ok, summary} = Generator.spawn_world(blueprint)

      crossroads = Procession.Entity.get_state("loc_crossroads")

      assert crossroads.metadata.exits == [
               %{to: "loc_briar_village", label: "village road"},
               %{to: "loc_silent_mine", label: "mine road"}
             ]
  ...
  describe "generate_world_ai/1" do
