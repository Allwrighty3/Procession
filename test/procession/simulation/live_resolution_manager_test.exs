defmodule Procession.Simulation.LiveResolutionManagerTest do
  ...
  test "manager owns resolution metadata without spawning entity processes" do
    ...
    before_entities = Procession.EntitySupervisor.list_entities() |> Enum.map(&elem(&1, 0)) |> MapSet.new()
    ...
    assert {:ok, %{resolution: :inert}} = LiveResolutionManager.make_inert(:remote_valley, name)
    assert {:ok, %{resolution: :inert, tick: 25}} = LiveResolutionManager.advance(:remote_valley, 25, [], name)
    assert {:ok, %{resolution: :live, entity_count: 2}} = LiveResolutionManager.refine(:remote_valley, 42, [], name)

    after_entities = Procession.EntitySupervisor.list_entities() |> Enum.map(&elem(&1, 0)) |> MapSet.new()
