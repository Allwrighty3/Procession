defmodule Procession.Simulation.RegionActivationLifecycleTest do
  ...
  test "deactivation retains only bounded social identities and activation restores them" do
    ...
    assert {:ok, _} = EntitySupervisor.start_entity(anchor, %{name: "Remembered Mara", type: :npc, traits: %{patience: 0.7}, metadata: %{identity_marker: :preserve_me}})
    ...
    region = MultiResolutionRegion.new(
      id: region_id,
      tick: 12,
      entities: [physical(anchor, {1, 1}, 0.2), physical(partner, {2, 1}, 0.1), physical(unanchored, {3, 1}, 0.05)],
      resources: [%{id: unique("store"), position: {4, 4}, quantity: 2.0}],
      social_relations: %{relation_key => %{expectation: 0.7, confidence: 0.8, persistence: 1.2, extreme_imprint: 0.0, exposure: 5.0, last_tick: 12}},
      causal_flags: [:old_flood]
    )

    assert {:ok, _} = LiveResolutionManager.put(region)
    assert {:ok, %{resolution: :inert, entity_count: 0}} = RegionActivationLifecycle.deactivate(region_id)
    ...
    assert archive.population_minds.population == 0

    assert {:ok, %{resolution: :inert, tick: 17}} = LiveResolutionManager.advance(region_id, 5)
    assert {:ok, %{resolution: :live, entity_count: 3}} = RegionActivationLifecycle.activate(region_id, 41)
  ...
  test "unanchored developmental minds survive through bounded population reconstruction" do
    ...
    assert {:ok, _} = EntitySupervisor.start_entity(entity_id, %{name: "Developing Mind", type: :npc, sensorimotor: [micro_nodes: 64, input_width: 4, seed: 19]})

    Enum.each(1..12, fn _ ->
      assert {:ok, _} = EntitySupervisor.sensorimotor_observe(entity_id, [{:signal, :scarcity, 4.0}], extreme_salience_threshold: 2.0)
    end)

    assert {:ok, before_trace} = EntitySupervisor.sensorimotor_trace(entity_id)
    assert before_trace.salience.imprint_count > 0

    region = MultiResolutionRegion.new(id: region_id, entities: [physical(entity_id, {1, 1})], resources: [])
    assert {:ok, _} = LiveResolutionManager.put(region)

    assert {:ok, %{resolution: :inert}} = RegionActivationLifecycle.deactivate(region_id, population_mind_cohort_limit: 2)
    refute EntitySupervisor.exists?(entity_id)

    assert {:ok, archive} = RegionActivationLifecycle.archive(region_id)
    assert archive.snapshots == %{}
    assert archive.mind_snapshots == %{}
    assert archive.population_minds.population == 1
    assert length(archive.population_minds.cohorts) == 1

    assert {:ok, %{resolution: :live, entity_count: 1}} = RegionActivationLifecycle.activate(region_id, 77)
    assert {:ok, refined} = LiveResolutionManager.fetch(region_id)
  ...
  test "activation collision leaves the inactive region and archive unchanged" do
    ...
    region = MultiResolutionRegion.new(
      ...
      resources: [],
      social_relations: %{{anchor, partner, :co_presence} => %{expectation: 0.5, confidence: 0.8, persistence: 1.0, extreme_imprint: 0.0, exposure: 3.0, last_tick: 0}}
    )
    ...
    assert {:error, {:entity_id_collisions, collisions}} = RegionActivationLifecycle.activate(region_id, 7)
    ...
    assert {:ok, %{resolution: :live, entity_count: 2}} = RegionActivationLifecycle.activate(region_id, 7)
