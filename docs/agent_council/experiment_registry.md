...
- **Design:** paired deterministic seeds `1..100` across four variants: C0 local-adaptive 90 pre/90 post, V1 local-adaptive 90 pre/180 post, V2 local-adaptive 180 pre/90 post, and V3 outcome-adaptive 90 pre/180 post. Behavioral correction uses a sliding 30-tick post-reversal window and reports the ending tick of the first qualifying window; non-overlapping action windows remain trajectory diagnostics. All behavioral/resistance correction, post-reversal obsolete-action, attribution, and metric-agreement labels are observer-only.
