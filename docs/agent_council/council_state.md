...
- `PermeableFlow` maps cognitive-field resistance into exit activation; the action sampler acts on exit activation rather than directly on residue.
...
## Iteration 003 bounded design

Iteration 003 contains two separately adjudicated diagnostics:

1. a read-only frozen-snapshot probe measuring residue → resistance → exit activation → normalized exit share → deterministic sampled-action frequency;
2. a matched scenario evaluation comparing the original endpoint reversal with a bounded interior relocation while preserving the learner-visible information boundary.

The iteration prospectively repairs net-support accounting using support units and supplements cumulative intake with local access, source-contact rate, and improvement-in-access rate.
...
5. Attribution eligibility may target support that is not decision-relevant.
6. A more general revision process may still be missing, but that conclusion remains deferred until translation and measurement adequacy are distinguished.
